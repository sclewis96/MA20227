...
subtitle: "A report on (linear) modelling for IMDb score prediction"
author:
...
knitr::opts_chunk$set(echo = TRUE, tidy = TRUE, cache = TRUE, fig.width = 8, eval=FALSE)
set.seed(2017)
```
...
3539*log(ms_out$rss[3]/3539) + 2*4 -
    (3539*log(ms_out$rss[9]/3539) + 2*10)
...
In the final section we briefly explored film genres and some trends that become apparent in the data when films are separated by genre -- for example, documentaries have undergone a recent surge in popularity and animations often have larger budgets than films of other genres.
