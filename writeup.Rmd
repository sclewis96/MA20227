...
We can interpret the coefficients of the model as a measure of the weight of each predictor in the model's output. In order for these weights to be comparable we must use predictors lying on the same scale; so we will now re-fit `rmod` on predictors rescaled on $[0, 1]$ (achieved by $x_{scaled} = \frac{x-\text{min}}{\text{max}}$).

```{r}
imde <- imdd
# Min-max scale numeric columns
imde[-c(1, 2, 10, 12, 13)] <- lapply(imdd[-c(1, 2, 10, 12, 13)],
                                     function(c) {(c-min(c))/max(c)})
rmod_sc <- lm(sqscore ~ year + durationsr + grosssr + budget + criticreviewssr +
               uservotessr + userreviewssr + grosssr:budget + ratingPG13, imde)
```
...
We unfortunately do not have the space in this report to adequately investigate whether the model performs well in general on unseen data. However we shall check a single case: Alfred Hitchcock's *Vertigo*.
...
In this specific case, `rmod` is virtually spot-on with its prediction, and even the simplistic `tmod` makes a prediction 0.7 of the actual score. Although this accuracy is encouraging, we reiterate that much more test data would be needed to carry out a thorough assessment of the model's performance.
